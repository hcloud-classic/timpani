import re
import subprocess
from .systemutil import Systemutil
...
class MountUtil(object):
    ...
    def collect_check_mount(self, nfs_server):
        try:
            cmd = "mount | awk '{print $1, $3}'"
            returncode, stdout = self.run(cmd)
            line = stdout.split('\n')
    ...
    def mount_exec(self, source, target):
        try:
            cmd = "mount -t nfs {} {}; sync".format(source, target)
    ...
    def umount_exec(self, target):
        try:
            cmd = "umount {}; sync".format(target)
            returncode, stdout = self.run(cmd)
        except Exception as e:
            logger.info("[mount_exec] EXCEPT : {}".format(e))
    ...
    def nfsumount(self, nfs_server, mount_path):
        devicelist, isnfsmount = self.collect_check_mount(nfs_server)
        if isnfsmount:
            self.umount_exec(mount_path)
        _, isnfsmount = self.collect_check_mount(nfs_server)
