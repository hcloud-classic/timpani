...
class _Command:
    ...
    def shell_run(shell_cmd, file_fd=None):
        ...
        try:
            if file_fd is None:
                output = subprocess.check_output(shell_cmd, universal_newlines=True, stderr=subprocess.PIPE, shell=True)
            else:
                output = subprocess.check_output(shell_cmd, universal_newlines=True, stderr=subprocess.PIPE, stdout=file_fd)
        except subprocess.CalledProcessError as e:
            raise e
