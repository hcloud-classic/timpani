...
class ZFS(object):
    ...
    def dev_list_linux():
        try:
            return _Command.shell_run('lsblk | grep disk | awk \'{print $1}\'')
        except subprocess.CalledProcessError as e:
            raise RuntimeError("Failed to get disk device")

    @staticmethod
    def dev_list_freebsd():
        try:
            return _Command.shell_run('geom disk list | grep Geom')
        except subprocess.CalledProcessError as e:
            raise RuntimeError("Failed to get disk device")

    @staticmethod
    def dev_boot_linux(devname):
        try:
            cmd = 'sfdisk -l {dev} | grep {dev} | grep \'*\' | '.format(dev=devname)
            cmd = cmd + "awk '{print $1, $2}'"
            return _Command.shell_run(cmd)
        except subprocess.CalledProcessError as e:
            raise RuntimeError("Failed to get disk device : {}".format(e))

    @staticmethod
    def dev_partition_dump_linux(dev_path, save_path):
        try:
            cmd = 'sfdisk -d {dev} > {save_path}'.format(dev=dev_path, save_path=save_path)
            return _Command.shell_run(cmd)
        except subprocess.CalledProcessError as e:
            raise RuntimeError("Failed to save partition :\ndev_path : {}\nsave_path : {}\n exception : {}".format(dev_path, save_path, e))

    @staticmethod
    def zdb_list_grep():
        try:
            return _Command.shell_run('zdb | grep \"name\\|type\\|id\\|ashift\\|path\"')
        except subprocess.CalledProcessError as e:
            raise RuntimeError("Failed to get zfs property '{property_target}' "
                               "from {pool}\n{e.output}\n")
    ...
    #
    #     if not mounted:
    #         if timpani_backup.check.check_valid_system() == "freebsd":
    #             call_args.append('-u')
    #         else:
    ...
    def zfs_send(send_target: str,
        ...
        if isfull:
            ...
            zfs_cmd = "zfs send -I {parent_target} {send_target} | ".format(parent_target=parent_target ,send_target=send_target)

        if isgzip:
            gzip_cmd = "gzip | "
            ssh_cmd = "ssh root@{target_host} \"cat > {remote_path}.zfs.gz\"".format(target_host=target_host,
                                                                                remote_path=remote_path)
            shell_cmd = zfs_cmd + gzip_cmd + ssh_cmd
        else:
            ssh_cmd = "ssh root@{target_host} \"cat > {remote_path}.zfs\"".format(target_host=target_host,
                                                                             remote_path=remote_path)
            shell_cmd = zfs_cmd + ssh_cmd
    ...
    @staticmethod
    def zfs_send_local(send_target: str,
                       parent_target: str,
                       remote_path: str,
                       isfull: bool = False,
                       isdata: bool = False,
                       isgzip: bool = True) -> str:
        ...
        if isfull:
            ...
            if parent_target is None:
                ...
                zfs_cmd = "zfs send -I {parent_target} {send_target} ".format(parent_target=parent_target,
                                                                            send_target=send_target)

        if isgzip:
            gzip_cmd = "| gzip "
            ssh_cmd = " > {remote_path}.zfs.gz".format(remote_path=remote_path)
            shell_cmd = zfs_cmd + gzip_cmd + ssh_cmd
        else:
            ssh_cmd = " > {remote_path}.zfs".format(remote_path=remote_path)
            shell_cmd = zfs_cmd + ssh_cmd
        logger.info("zfs cmd : {}".format(shell_cmd))
        try:
            _Command.shell_run(shell_cmd)
        except subprocess.CalledProcessError as e:
            raise RuntimeError("Failed to snapshot send {}\n {}\n".format(send_target, e))

    @staticmethod
    def zfs_restore_local(target: str,
                       remote_path: str,
                       isfull: bool = False,
                       isdata: bool = False,
                       isgzip: bool = True) -> str:
        ...
        if isdata:
            zfs_cmd = " zfs receive -F {target} ".format(target=target)
        else:
            zfs_cmd = " zfs receive -F {target} ".format(target=target)

        if isgzip:
            gzip_cmd = "| gunzip |"
            ssh_cmd = " cat {remote_path}".format(remote_path=remote_path)
            shell_cmd = ssh_cmd + gzip_cmd + zfs_cmd
        else:
            ssh_cmd = " cat {remote_path}".format(remote_path=remote_path)
            shell_cmd = zfs_cmd + ssh_cmd
        logger.info("zfs cmd : {}".format(shell_cmd))
        try:
            ...
            raise RuntimeError("Failed to snapshot send {}\n {}\n".format(target, e))
        logger.info("RESTOR RUN OUT : {}".format(output))
        return output
    ...
    @staticmethod
    def zpool_status_grep(pool_name, print_cnt):
        shell_cmd = "zpool status -L {} | grep -A{} NAME | ".format(pool_name, print_cnt)
        shell_cmd = shell_cmd + "awk '{print $1}'"
        logger.debug("cmd : {}".format(shell_cmd))
        try:
            output = _Command.shell_run(shell_cmd)
            logger.debug("output : {}".format(output))
            return output
        except subprocess.CalledProcessError as e:
            raise RuntimeError("Failed to zpool status failed\n{}\n".format(e))

    @staticmethod
    def zpool_status_grep_freebsd(pool_name, print_cnt):
        shell_cmd = "zpool status {} | grep -A{} NAME | ".format(pool_name, print_cnt)
        shell_cmd = shell_cmd + "awk '{print $1}'"
        logger.debug("cmd : {}".format(shell_cmd))
        try:
            output = _Command.shell_run(shell_cmd)
    ...
    @staticmethod
    def demoncheck(demonname):
        shell_cmd = "service {} status".format(demonname)
        logger.debug("cmd : {}".format(shell_cmd))
        try:
            output = _Command.shell_run(shell_cmd)
            logger.debug("output : {}".format(output))
    ...
    @staticmethod
    def mount_grep(grep_word):
        shell_cmd = "mount | awk '{print $1, $3}'"
        if grep_word is not None:
            grep_cmd = " | grep {}".format(grep_word)
            shell_cmd += grep_cmd
        logger.debug("cmd : {}".format(shell_cmd))
        try:
            output = _Command.shell_run(shell_cmd)
    ...
    @staticmethod
    def nfsmount(source, target):
        shell_cmd = "mount -t nfs {} {}; sync".format(source, target)
        logger.debug("cmd : {}".format(shell_cmd))
        try:
            output = _Command.shell_run(shell_cmd)
            logger.debug("output : {}".format(output))
    ...
    @staticmethod
    def nfsunmount(target):
        shell_cmd = "umount {}; sync".format(target)
        logger.debug("cmd : {}".format(shell_cmd))
        try:
            output = _Command.shell_run(shell_cmd)
            logger.debug("output : {}".format(output))
    ...
    @staticmethod
    def gpart_backup(device:str, target_host:str, remote_path:str):

        if platform.system() == 'FreeBSD':
            shell_cmd = "gpart backup {device} | ssh {target_host} \"cat > {remote_path}\"".\
                format(device=device, target_host=target_host, remote_path=remote_path)
        else:
            return None
    ...
    @staticmethod
    def gpart_backup_local(device: str, remote_path: str):

        if platform.system() == 'FreeBSD':
            shell_cmd = "gpart backup {device} > {remote_path}". \
                format(device=device, remote_path=remote_path)
        else:
            return None
    ...
    def zfs_list(target: str = None,
        ...
        try:
            ...
            raise RuntimeError("Failed to get zfs list of {}\n{}\n".format(target,e))
